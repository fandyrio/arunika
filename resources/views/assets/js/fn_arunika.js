...
function showLoading(){
    ...
    $(".modal-dialog").html("<center><img src='../resources/views/assets/image/loading.gif' width='30%'></center>");
}
...
function readURLFileSize(input, type, className){
    ...
    var allowedType=["application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/pdf", "image/png", "image/jpeg", "image/jpg"];
    if(allowedType.includes(input.files[0].type) && input.files[0].size <= 6291456){
        $(".filename_baru").html("<br />File baru : "+input.files[0].name);
    }else{
        callSwal("error", "Tipe data harus *.doc / *.docx / *.pdf / *.jpg / *.png dan Maximum Ukuran File 6mb", true);
        $("."+className).val("");
    }
    reader.readAsDataURL(input.files[0]); // convert to base64 string
}
